...
//     CHECK(acceptor.get_recv_io_state().get_error() == corlib::error_code::NONE_ERROR);
//     buffer_t rbuf, sbuf;
//     auto rs = co_await recv_task_t(new_con, rbuf);
//     CHECK(rs == 5);
//     CHECK(new_con.get_recv_io_state().get_error() == corlib::error_code::NONE_ERROR);
//     rbuf.go_to_start();
...
{
  static const int connections_count = 3;
  ...
  auto loop_client = [&](char const *addr) {
    ...
    auto recv_task = [&](io_context_t& io_ctx, atomic_size_t& total_recv2, shared_ptr<connection_t> pcon) -> task_t<void> {
      ...
      while (true)
      {
        ...
        total_recv2 += rl;
        if (pcon->get_recv_io_state().get_error() != corlib::error_code::NONE_ERROR)
          break;
      }
  ...
  auto loop_send_recv = [&]() {
    ...
    auto recv_task = [&](shared_ptr<connection_t> con) -> task_t<void> {
      ...
      while (true)
      {
        ...
        total_recv1 += rl;
        if (con->get_recv_io_state().get_error() != corlib::error_code::NONE_ERROR)
        {
          break;
        }
      }
    ...
    auto accept_task = [&]() -> task_t<void> {
      ...
      {
        connection_t new_con = co_await accept_task_t(io_ctx, acceptor);
        if (acceptor.get_recv_io_state().get_error() == corlib::error_code::NONE_ERROR)
        {
